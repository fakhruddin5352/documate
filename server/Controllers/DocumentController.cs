...
namespace Documate.Controllers {
    ...
    public class DocumentController : ControllerBase {
        ...
        public async Task<IActionResult> DownloadRequest ([FromQuery] DownloadRequest request) {
            //check if 
            try {
                var data = await documentService.Load (request.Sender, request.Signature, request.Hash);
                var mime = MimeTypes.GetMimeType (data.Name);
                EntityTagHeaderValue entityTag = EntityTagHeaderValue.Parse ("\"" + data.Hash.RemoveHexPrefix () + "\"");
        ...
        public async Task<IActionResult> UploadRequest ([FromForm] UploadRequest request) {
            ...
            using (var stream = new MemoryStream (data)) {
                ...
                try {
                    var info = await documentService.Store (request.Sender, request.Signature, data, request.Name);
